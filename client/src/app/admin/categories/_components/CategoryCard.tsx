...
import { useCreateSubcategory, useUpdateCategory, useDeleteCategory, useUpdateSubcategory, useDeleteSubcategory } from "@/hooks/useCategory";
import { useQueryClient } from "@tanstack/react-query";
import { toast } from "sonner";
import { CategoryEditDialog } from "@/components/dialog/CategoryEditDialog";

export function useMutationWithToast(hook: any) {
...
export default function CategoryCard({ category }: CategoryCardProps) {
  ...
  const updateCategory = useMutationWithToast(useUpdateCategory);
  const deleteCategory = useMutationWithToast(useDeleteCategory);
  const createSub = useMutationWithToast(useCreateSubcategory);
  const updateSub = useMutationWithToast(useUpdateSubcategory);
  const deleteSub = useMutationWithToast(useDeleteSubcategory);
